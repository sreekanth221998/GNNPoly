...
class GNNGH_T(nn.Module):
    def __init__(self, v_in, e_in, u_in, hidden_dim):
        super(GNNGH_T, self).__init__()
        
        self.graphnet1solv = gnn.MetaLayer(EdgeModel(v_in, e_in, u_in, hidden_dim),
                                      NodeModel(v_in, u_in, hidden_dim), 
                                      GlobalModel(u_in, hidden_dim))
        
        self.graphnet1solu = gnn.MetaLayer(EdgeModel(v_in, e_in, u_in+1, hidden_dim),
                                      NodeModel(v_in, u_in+1, hidden_dim), 
                                      GlobalModel(u_in+1, hidden_dim))
    ...
    def forward(self, solvent, solute, T):
        ...
        # -- Extra feature for the polymer
        ext2 = solute.ext.reshape(-1,1)
        
        u1 = torch.cat((ap1,bp1,topopsa1), axis=1) # Molecular descriptors solvent
        u2 = torch.cat((ap2,bp2,topopsa2,ext2), axis=1) # Molecular descriptors solute
        ...
        if solvent.edge_attr.shape[0] == 0 or solute.edge_attr.shape[0] == 0:
            ...
            solute = cat_dummy_graph(solute, solute_bool=True)
            u1_dummy = torch.tensor([1,1,1]).reshape(1,-1)
            u2_dummy = torch.tensor([1,1,1,1]).reshape(1,-1)
            if torch.cuda.is_available():
                u1_dummy = u1_dummy.to(torch.device('cuda' if torch.cuda.is_available() else 'cpu'))
                u1_dummy = u1_dummy.cuda() 
                u2_dummy = u2_dummy.to(torch.device('cuda' if torch.cuda.is_available() else 'cpu'))
                u2_dummy = u2_dummy.cuda() 
        ...
        # Solvent GraphNet
        x1, edge_attr1, u1 = self.graphnet1solv(solvent.x, 
                                           solvent.edge_index, 
                                           solvent.edge_attr, 
                                           u1, 
                                           solvent.batch)
        ...
        # Solute GraphNet
        x2, edge_attr2, u2 = self.graphnet1solu(solute.x, 
                                           solute.edge_index, 
                                           solute.edge_attr, 
                                           u2, 
                                           solute.batch) 
