...
def sys2graph(df, mol_column_1, mol_column_2, target, extra, y_scaler=None, single_system=False):
    '''
    ...
    if single_system:
        ...
        ys   = df[target].tolist()
        exts  = df[extra].tolist()
    
    graphs_solvent, graphs_solute = [], []
    for y, solv, solu, ext in tqdm(zip(ys, solvents, solutes, exts), total=len(ys)):
...
def get_dummy_graph(solute_bool):
    ...
    edges_info_solv = torch.tensor(np.array(edge_attr_solv), dtype=torch.float)
    if solute_bool:
        ap, bp, topopsa, hb, inter_hb, y, ext = [torch.tensor([1])]*7
        graph_dummy = Data(x=nodes_info_solv, edge_index=edges_indx_solv, edge_attr=edges_info_solv,
                           ap=ap, bp=bp, topopsa=topopsa, hb=hb, inter_hb=inter_hb, y=y, ext=ext)
    else:
...
def cat_dummy_graph(batch, solute_bool=False):
    dummy_graph = get_dummy_graph(solute_bool)
    ...
    if solute_bool:
        batch.ext = torch.cat((batch.ext, dummy_graph.ext))
